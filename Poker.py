...
def make_dist(n):
    ...
    prob_nothing = (nothing / n) * 100
    prob_pair = (pair / n) * 100
    prob_flush = (flush / n) * 100
    prob_straight = (straight / n) * 100
    prob_three_of_a_kind = (three_of_a_kind / n) * 100
    prob_straight_flush = (straight_flush / n) * 100
    probabilities = {'Pair':prob_pair,'Nothing':prob_nothing,'Flush':prob_flush,'Straight':prob_straight,'Straight Flush':prob_straight_flush,'Three of a Kind':prob_three_of_a_kind}
...
def play_round(dealer_hand, player_hand, cash, get_ante, is_playing):
    ...
    """
    bet = get_ante(cash)
    play_dec = is_playing(player_hand)
    if play_dec == False:
        return (bet,-1)
    elif play_dec == True:
        if dealer_hand < min_dealer_hand:
            return (bet,1)
        elif dealer_hand > min_dealer_hand:
